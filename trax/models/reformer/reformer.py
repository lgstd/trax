...
class Map(tl.Layer):
  ...
  def forward_with_state(self, inputs, weights=(), state=(), rng=None):
    if self._n_sections == 1:
      results = self._layer(inputs, w=weights, s=state, r=rng)
    else:
      rngs = _split_rngs(rng, len(inputs))
      results = [self._layer(x, w=weights, s=state, r=r)
...
class ReversibleHalfResidual(tl.ReversibleLayer, tl.Serial):
  ...
  def reverse_and_grad(self, output, ct, weights=(), state=(), new_state=(),
    ...
    def call_compute_residual(x, weights):
      res = self.compute_residual(x, w=weights, s=state[0], r=rngs[0])
      return res
    ...
    reconstructed_x = self.subtract_top(
        stack_with_residual, w=weights[-1], s=state[-1], r=rngs[-1])
...
class ReversibleAttentionHalfResidual(tl.ReversibleLayer, tl.Serial):
  ...
  def reverse_and_grad(self, output, ct, weights=(), state=(), new_state=(),
    ...
    def call_pre_attention(x, weights):
      res = self.pre_attention(x, w=weights, s=state[0], r=rngs[0])
      return res
    ...
    def call_post_attention(x):
      res = self.post_attention(x, w=weights[2], s=state[2], r=rngs[2])
      return res
    ...
    def call_post_attention2(weights):
      res = self.post_attention(stack, w=weights, s=state[2], r=rngs[2])
      return res
    ...
    reconstructed_x = (
        self.subtract_top(stack, w=weights[-1], s=state[-1], r=rngs[-1]))
